...
$params = array('reportDescription' => array(
    ...
    'dateTo' => date('Y-m-d', strtotime($date .' -0 day')), //  the end date of the report range, format YYYY-MM-DD
    'metrics' => array(
        array('id' => 'event1'),
        array('id' => 'orders'),
        array('id' => 'visits'),
            ...
            if ($isDone) {
                ...
                if ($report !== false) {
                    while (!$is_successfull) {
                        $is_successfull = $reportController->saveToDb($report, "item_id", $argv[2]);
                    }
                }
            } else $isDone = false;
            $whilecount++;
