...
/*
$Id: nusoap.php,v 1.1 2011/01/18 02:19:24 Jeff Exp $
...
* @version  $Id: nusoap.php,v 1.1 2011/01/18 02:19:24 Jeff Exp $
* @access   public
*/
class nusoap_base {
	/**
	...
	function parseString($xml,$type){
		...
		if($xml != ""){
			...
			// Display an error message.
				$errstr = sprintf('XML error parsing XML schema on line %d, byte %d: %s',
	...
	*/
	function soap_parser($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		...
		if(!empty($xml)){
			...
			if(!xml_parse($this->parser,$xml,true)){
			    // Display an error message.
			    $err = sprintf('XML error parsing SOAP payload on line %d, byte %d: %s',
			    xml_get_current_line_number($this->parser),
				xml_get_current_byte_index ( $this->parser ),
			    xml_error_string(xml_get_error_code($this->parser)));
				$this->debug($err);
